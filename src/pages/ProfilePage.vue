              ...
              <q-item-section side>
                ...
                <q-menu touch-position>
                  <q-item v-close-popup style="max-width: 400px;">
                    <q-item-section>Не знал как на странице профиля реализовать полноценный CRUD, т.к. информация о юзере после регистрации уже создана (запись в таблице есть, но поля не заполнены). В таком случае заполнение или очистка полей таблицы users больше подходит под update. Поэтому, для полноценного CRUD я сделал секцию со списком информации "о себе", где реализованы полноценные create и delete</q-item-section>
                  </q-item>
                </q-menu>
              </q-item-section>
            </q-item>
...
export default {
  setup() {
    ...
    return {
      ...
      addInfo() {
        api.post('/users/create_info', { new_info: new_info.value }).then((result) => {
